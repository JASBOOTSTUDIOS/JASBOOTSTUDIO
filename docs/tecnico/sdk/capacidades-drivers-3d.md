...
**Fecha:** 2026-03-24. *Una hoja de seguimiento con ítems marcables vive solo en el monorepo local (`docs/TECNICO/SDK/CHECKLIST_ESTABILIDAD_DRIVERS_3D.md`); aquí solo se resume el estado.*
...
- **Heap:** `reservar(n)` / `liberar(ptr)` en fuente → `OP_HEAP_RESERVAR` / `OP_HEAP_LIBERAR` en jbc; detalle en [heap-memoria.md](heap-memoria.md)
...
- Convención de registros, hasta 4 argumentos, retorno **entero o puntero (64b)**; **retorno flotante** como extensión futura (ver [ffi-convencion-llamada.md](ffi-convencion-llamada.md))
...
| Punteros como tipo (`puntero a entero`, etc.) | Pendiente. |
| `inb`/`outb`, `readl`/`writel` o equivalentes | No. |
| Interrupciones, DMA, timers de kernel | No. |
| Atómicos / lock-free | No (D.7 opcional). |

**Conclusión:** para drivers reales hace falta avanzar en punteros, MMIO y, si aplica, un target kernel o bare metal.
...
| Ejemplo mínimo documentado (“hola OpenGL”) | Pendiente. |
| Límites del IR documentados (pila, heap máx., FFI) | Pendiente. |
...
1. **Drivers:** cerrar punteros, MMIO y modelo de target si el objetivo es hardware o kernel.
2. **Motor 3D:** ejemplo mínimo con FFI y documentar límites de pila, heap y FFI; valorar retorno flotante en FFI si simplifica bindings.
3. **Seguimiento interno:** el desglose con casillas de seguimiento se mantiene en el monorepo (`docs/TECNICO/SDK/CHECKLIST_ESTABILIDAD_DRIVERS_3D.md`), fuera de este repositorio público.
