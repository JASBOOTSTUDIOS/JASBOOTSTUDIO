...
| `consolidar_memoria()` / `dormir()` / `consolidar()` | Ejecuta el ciclo **decaimiento → olvido de débiles → consolidación** (refuerzo multiplicativo fijo del **5%** sobre supervivientes; ver `../tecnico/vm/vm-estable.md`). |
...
| `percepcion_recientes()` | Devuelve el ID de una **lista** en memoria episódica (orden reciente → antiguo); ver `../tecnico/vm/vm-estable.md`. |
...
| `rastro_activacion_lista()` / `rastro_activacion_recientes()` | ID de lista episódica (orden viejo → nuevo); ver `../tecnico/vm/vm-estable.md`. |
...
Elegir un elemento de una **lista** (colecciones RAM) según la **fuerza** en el JMN entre un **contexto** y cada candidato. Ver `../tecnico/vm/vm-estable.md`.
...
Llamar código en DLL/SO (convención de registros y límites: [ffi-convencion-llamada.md](../tecnico/sdk/ffi-convencion-llamada.md)):
