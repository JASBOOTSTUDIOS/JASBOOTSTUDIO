...
└── docs/                        # Documentación del paquete (no sustituye docs globales del repo)
    ...
    ├── desarrollo/              # Planes y notas internas del paquete (no sustituye docs globales)
    └── guias/                   # Guías de uso
```
...
| `docs/` | Texto del equipo sobre **este** paquete | enlazar a la referencia del lenguaje y al índice del repo de documentación si aplica |
...
## Pautas al crear un paquete nuevo

- **Estructura:** existen `src/`, `tests/`, `data/` (aunque `data/` esté vacío con un `.gitkeep`).
- **README:** en la raíz del paquete, con el comando mínimo de compilación y ejecución.
...
- **`usar`:** rutas revisables sin más de dos o tres `../` entre `tests/` y `src/`.
...
- Crecer `docs/` sin índice (`README.md`) cuando hay más de tres documentos.

---

## Relación con el resto del repositorio Jasboot

- **Sintaxis y semántica** del lenguaje: [referencia-lenguaje.md](referencia-lenguaje.md).
- **Estrategia y calidad de tests** a escala toolchain (VM, compilador, integración): documentación interna del monorepo (`docs/PROYECTO/CHECKLIST_TESTS_PRODUCCION_JASBOOT.md` y afines); no se publica en este repositorio.
